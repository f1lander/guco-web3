...
export default function BeginnerLessonPage() {
  ...
  const [lesson, setLesson] = useState<typeof LESSONS.beginner[0] | undefined>();
  
  useEffect(() => {
    setMounted(true);
    setLesson(LESSONS.beginner.find(l => l.id === lessonId));
  }, [lessonId]);
  ...
  if (!lesson) {
    return (
      <div className="flex flex-col gap-20 p-8">
        <h1 className="text-4xl font-bold text-gray-900 dark:text-white">Lección no encontrada</h1>
        <button 
  ...
  if (!isLessonUnlocked(lesson.id)) {
    return (
      <div className="flex flex-col gap-20 p-8">
        <h1 className="text-4xl font-bold text-gray-900 dark:text-white">Lección bloqueada</h1>
  ...
  const handleStartLevel = () => {
    router.push(`/dashboard/level?levelData=${encodeURIComponent(lesson.levelData)}&levelId=${lesson.levelId}`);
  };
