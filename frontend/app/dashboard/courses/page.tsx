...
const DifficultyCard: React.FC<{
  ...
  return (
    <div
      className={`rounded-lg p-6 bg-white/5 dark:bg-gray-800/50 backdrop-blur-sm border border-gray-200 dark:border-gray-700 ${!isUnlocked ? "opacity-50" : ""}`}
    >
...
const CoursesPage: React.FC = () => {
  ...
  React.useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (
        firstBeginnerLesson &&
        !unlockedLessons.includes(firstBeginnerLesson.id)
      ) {
  ...
  const isIntermediateUnlocked = LESSONS.beginner.every((lesson) =>
  ...
  const isAdvancedUnlocked =
    ...
    LESSONS.intermediate.every((lesson) => isLessonUnlocked(lesson.id));
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <DifficultyCard
          ...
          onLessonClick={(lessonId) =>
            handleLessonClick("intermediate", lessonId)
          }
        />
