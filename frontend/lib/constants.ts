...
export const DEFAULT_NETWORK_NAME = (
  Object.entries(NETWORK).find(([, id]) => id.chainId === CHAIN_ID)?.[0] || ""
...
export const COMMAND_CATEGORIES = {
  ...
  acciones: {
    label: "actions",
    color: "yellow",
    commands: [{ command: COMMANDS.collect_item, description: "collect_item" }],
  },
  control: {
    label: "control",
    color: "purple",
    commands: [
      {
        command: COMMANDS.variable_assign,
        description: "set_variable",
        type: "variable",
      },
      {
        command: COMMANDS.for_loop,
        description: "repeat_command",
        type: "loop",
      },
    ],
  },
