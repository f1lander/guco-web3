...
const BlocklyEditor: React.FC<BlocklyEditorProps> = ({ value, onChange }) => {
  ...
  const [activeCategory, setActiveCategory] =
    useState<keyof typeof COMMAND_CATEGORIES>("basico");
    ...
    const lines = code
      ...
      .filter(
        (line) =>
          line &&
          !line.startsWith("--") &&
          line !== "-- Area de codigo para programar el robot" &&
          !line.includes("-- class definition"),
      );

    const blocks: Block[] = [];
    const blockStack: { blocks: Block[]; type: string }[] = [
      { blocks, type: "root" },
    ];
  ...
  const getBlockColor = (command: string, blockType: Block["type"]): string => {
    if (blockType === "variable") {
      ...
      command.includes("encender") ||
      command.includes("apagar") ||
      command.includes("Robot.new")
    ) {
  ...
  const renderInput = (block: Block, inputName: string, label: string) => {
    const isEditing =
      editingBlock?.id === block.id && editingBlock?.input === inputName;
  ...
  const renderBlock = (block: Block, isNested: boolean = false) => {
    ...
    return (
      <div
        ...
        draggable
        onDragStart={(e) =>
          handleDragStart(e, block.command, block.type, block.id)
        }
        ...
        <div className="flex items-center justify-between p-1 text-white">
          {block.type === "variable" ? (
            <div className="font-mono text-xs md:text-sm flex items-center">
              {renderInput(block, "name", "Variable")} ={" "}
              {renderInput(block, "value", "Value")}
            </div>
          ) : block.type === "loop" ? (
            <div className="font-mono text-xs md:text-sm flex items-center">
              for {renderInput(block, "iterator", "Iterator")}=
              {renderInput(block, "start", "Start")},
              {renderInput(block, "end", "End")} do
            </div>
          ) : (
      ...
      <div className="flex flex-wrap gap-1 p-2 bg-slate-800 border-b border-slate-700">
        {Object.entries(COMMAND_CATEGORIES).map(([key, category]) => (
          <Button
            ...
            onClick={() =>
              setActiveCategory(key as keyof typeof COMMAND_CATEGORIES)
            }
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-1/3 bg-slate-800/50 border-r border-slate-700 p-2 overflow-y-auto">
          <h3 className="text-xs font-semibold text-slate-400 mb-2">
            {COMMAND_CATEGORIES[activeCategory].label}
          </h3>
          <div className="flex flex-col gap-1">
            {COMMAND_CATEGORIES[activeCategory].commands.map((cmd, index) => {
              ...
              const blockType = commandStr.includes("for")
                ? "loop"
                : commandStr.includes("if")
                  ? "if"
                  : commandStr.includes("else")
                    ? "else"
                    : commandStr.includes("=") && !commandStr.includes("for")
                      ? "variable"
                      : "command";
