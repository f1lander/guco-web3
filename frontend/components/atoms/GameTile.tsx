...
export const GameTile = ({ type, onClick, editable, className, robotState, isColliding = false }: GameTileProps) => {
  ...
  return (
    <div className={`relative bg-slate-700/50 border border-slate-600/30 min-h-[2rem] md:min-h-[3rem] flex items-center justify-center hover:border-blue-500/50 transition-colors ${className}`}>
      {editable ? (
        <Select
          value={type.toString()}
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          ...
          {(showRobot || !isColliding) && (
            <span className={`text-2xl md:text-3xl transition-all duration-300 ${getRobotClass()}`}>
              {getEmoji(type)}
            </span>
          )}
          ...
          {type === TileType.ROBOT && isColliding && (
            <span className={`absolute z-10 transition-all duration-300 ${getExplosionClass()}`}>
              💥
            </span>
          )}
        </div>
