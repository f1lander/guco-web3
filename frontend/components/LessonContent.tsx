  ...
  const renderSection = (section: string) => {
    ...
    return (
      <div key={header} className="mb-12">
        ...
        {headerLevel === 1 && (
          <h1 className="text-4xl font-mono font-bold mb-6 text-gray-100">{headerText}</h1>
        )}
        {headerLevel === 2 && (
          <h2 className="text-3xl font-mono font-bold mb-4 text-gray-200">{headerText}</h2>
        )}
        {headerLevel === 3 && (
          <h3 className="text-2xl font-mono font-bold mb-4 text-gray-300">{headerText}</h3>
        )}
          ...
          {contentLines.map((line, index) => {
            ...
            if (line.startsWith('```')) {
              if (!inCodeBlock) {
                ...
                currentCodeBlock = [];
                return (
                  <pre key={index} className="bg-[#0d1117] rounded-lg p-4 overflow-x-auto border border-gray-700/50">
              ...
              return (
                <p key={index} className="text-gray-300">
                  {line.split('**').map((part, i) => 
                    i % 2 === 0 ? part : <strong key={i} className="font-bold text-gray-100">{part}</strong>
                  )}
                </p>
              );
            }
            ...
            if (line === '#') {
              return <hr key={index} className="my-8 border-t border-gray-700" />;
            }
